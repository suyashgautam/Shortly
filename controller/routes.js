...
router.post('/create', checkAuth, (req, res) => {
    ...
    if (!original || !short) {
        ...
        urls.findOne({ slug: short }, (err, data) => {
            if (err) throw err;
            if (data) {
                urls.find({ owned : req.user.email }, (err, urls) => {
                    if(err) throw err;
                    
                    res.render('dashboard', { verified: req.user.isVerified, logged: true, csrfToken: req.csrfToken(), err: "Try Different Short Url, This exists !" , urls: urls});
